...
ELS.forEach(([num, sym, name, cat, col, row, mass]) => {
  ...
  d.setAttribute("aria-label", `${name}, element ${num}, ${CATS[cat].label}`);
  d.innerHTML = `<span class="n" aria-hidden="true">${num}</span><span class="s">${sym}</span><span class="nm" aria-hidden="true">${name}</span><span class="ms" aria-hidden="true">${mass}</span>`;
  const open = () => open_(num, sym, name, cat, mass);
  d.onclick = open;
  d.onkeydown = (e) =>
...
Object.entries(CATS).forEach(([k, { label, c }]) => {
  const d = document.createElement("div");
  d.className = "li";
  d.innerHTML = `<div class="ld" style="background:${c}20;border-color:${c}77" aria-hidden="true"></div><span>${label}</span>`;
  leg.appendChild(d);
});
...
function open_(num, sym, name, cat, mass) {
  ...
  mc.textContent = CATS[cat].label;
  mc.style.cssText = `color:${c};border-color:${c}66;background:${c}18`;
